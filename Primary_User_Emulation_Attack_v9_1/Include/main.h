...
#define NetSimxmlError xmlErrorRedefine
# include "Stack.h"
# include "Packet.h"
...
/* Used to check if dll's is modified by user or not */
static bool CHECK_FOR_USER_DLL(const char* dllName) 
{
	HINSTANCE hdll;
	FARPROC f=NULL;
	...
	if(hdll)
	{
		f = GetProcAddress(hdll,"fn_NetSim_Check_for_user_dll");
	}
	if(f)
		return true;
	else
	{
		fprintf(stderr,"\n********************\n");
		fprintf(stderr,"WARNING:\nDetected a change in %s\n",dllName);
		fprintf(stderr,"This message is normally shown if users link their own code to NetSim.\n");
		fprintf(stderr,"********************\n");
